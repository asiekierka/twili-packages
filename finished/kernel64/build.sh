...
make CC=gcc allnoconfig ARCH=x86_64 HOSTCFLAGS="-D_GNU_SOURCE" KCONFIG_ALLCONFIG=../linux.config
#make i386_defconfig HOSTCFLAGS="-D_GNU_SOURCE"
make CC=gcc HOSTCFLAGS=-D_GNU_SOURCE ARCH=x86_64 INSTALL_HDR_PATH=dest headers_install
find dest/include \( -name .install -o -name ..install.cmd \) -delete
mkdir -p $1/include/
cp -rv dest/include/* $1/include/

make CC=gcc ARCH=x86_64 HOSTCFLAGS=-D_GNU_SOURCE CFLAGS="" LDFLAGS=""
make CC=gcc INSTALL_MOD_PATH=$1 ARCH=x86_64 modules_install
